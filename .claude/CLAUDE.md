...
**Online-model routing logic** (`app/api/chat/route.ts` + `lib/responseAnalyzer.ts`): before calling any model, the route checks the user's latest message with `needsLiveData()` — a keyword/pattern scan for recency signals ("today", "latest", "news", "score", "weather", a near-present year, etc.). A match routes to `MODELS.ONLINE` (`openai/gpt-5:online`, OpenRouter's web-connected variant); otherwise it uses `MODELS.BASE` (`openai/gpt-5`). Either way, the chosen model's result is streamed straight to the client via `toUIMessageStreamResponse()` with no upfront buffering. This is deliberate: an earlier version buffered the base model's full response first (to scan it for hedging phrases like "knowledge cutoff" before deciding whether to retry against the online model) — even reusing the same `streamText()` result via its `teeStream()`-backed accessors still forces the whole response into memory before any of it reaches the browser, defeating live token-by-token delivery for every non-recency message (verified: 1 giant chunk at the very end instead of hundreds of chunks over several real seconds). Don't reintroduce a "read the full response, then decide" step before streaming to the client — do recency detection on the query upfront instead.
...
**Visual language**: both `app/page.tsx` (landing) and the chat UI share a deliberate "technical/enterprise" styling convention — `font-mono uppercase tracking-widest` micro-labels (e.g. "MODEL", "RECENT", "THEME"), a small emerald `animate-pulse` dot for "live" status indicators, and `public/chadgpt-logo.svg` as the one consistent brand mark (nav, sidebar header, empty-state, and `app/icon.svg` for the browser tab). Match this when adding new UI rather than introducing a different label/accent style.

**Path alias**: `@/*` maps to the repo root (see `tsconfig.json`).

## Code Style

Comments (line `//`, block `/* */`, or JSDoc `/** */`) must not exceed 2 lines total, including opening/closing delimiters. Shorten to the essential point rather than writing a longer comment.
